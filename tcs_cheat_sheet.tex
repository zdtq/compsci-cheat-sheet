...
\begin{multicols*}{4}
        ...
        \begin{minipage}{\textwidth}
                ...
                \begin{tabular}{r c c c c c c c}
                                                & \textbf{SF} & \textbf{REG} & \textbf{DCFL} & \textbf{CFL} & \textbf{CSL} & \textbf{RIC} & \textbf{RE} \\ \hline
                        \texttt{Unione}         & \checkmark & \checkmark   & $\times$      & \checkmark   & \checkmark   & \checkmark   & \checkmark  \\\hline
                        \texttt{Intersezione}   & \checkmark & \checkmark   & $\times$      & $\times$     & \checkmark   & \checkmark   & \checkmark  \\\hline
                        \texttt{Differenza}     & \checkmark & \checkmark   & $\times$      & $\times$     & \checkmark   & \checkmark   & $\times$    \\\hline
                        \texttt{Complemento}    & \checkmark & \checkmark   & \checkmark    & $\times$     & \checkmark   & \checkmark   & $\times$    \\\hline
                        \texttt{Concatenazione} & \checkmark & \checkmark   & $\times$      & \checkmark   & \checkmark   & \checkmark   & \checkmark  \\\hline
                        \texttt{Star di Kleene} & $\times$ & \checkmark   & $\times$      & \checkmark   & \checkmark   & \checkmark   & \checkmark  \\\hline
                \end{tabular}
                \egroup
                \vspace{5pt}
        \end{minipage}
        \begin{tabular}{@{}r p{7.6cm}@{}}
                \verb|SF|   & Linguaggi star free (esprimibili mediante una logica del prim'ordine) \\
                \verb|REG|  & Linguaggi regolari (esprimibili mediante un'espressione regolare / riconosciuti da una grammatica regolare o da un FSA) \\
                \verb|DCFL| & Linguaggi context free deterministici (riconosciuti da un automa a pila deterministico)          \\
                \verb|CFL|  & Linguaggi context free (riconosciuti da una grammatica context free o da un automa a pila non deterministico)                                        \\
                \verb|CSL|  & Linguaggi context sensitive (riconosciuti da una grammatica context sensitive)                              \\
                \verb|RIC|  & Linguaggi ricorsivi (esiste una MT che per ogni stringa in input, termina la sua esecuzione determinando se la stringa è accettata o rifiutata)      \\
                \verb|RE|   & Linguaggi ricorsivamente enumerabili (riconosciuti da una grammatica non ristretta o da una MT
    che può non terminare sugli input non appartenenti al linguaggio)                         \\
        \end{tabular} \\ [5pt]
        \verb|SF| $\subset$ \verb|REG| $\subset$ \verb|DCFL| $\subset$ \verb|CFL| $\subset$ \verb|CSL| $\subset$ \verb|RIC| $\subset$ \verb|RE|\\ [3pt]
        \vspace{-0.3cm}
        ...
        Sia $F = \{f_i\ |\ i \in \mathbb{N}\}$ l'insieme di tutte le funzioni computabili, sia $P \subseteq F = \{f_i\ |\ f_i\text{ soddisfa una certa proprietà}\}$ un sottoinsieme di $F$ di funzioni computabili che soddisfano una certa proprietà e sia \\$S=\{x\ |\ f_x \in P\}$ l'insieme degli indici delle funzioni che appartengono a $P$. Allora $S$ ric. $\iff P = \varnothing \lor P = F$. Più informalmente, il teorema di Rice afferma che per qualsiasi proprietà non banale (cioè vera per almeno una funzione e falsa per almeno una funzione) delle funzioni computabili, non esiste un algoritmo che possa decidere se una data funzione computabile soddisfi o meno quella proprietà.
                ...
                \textbf{Caso decidibile} Sia $P_1$ un noto problema decidibile e sia $P_2$ un problema che si sospetta essere decidibile. Si può dimostrare che $P_2$ è decidibile riducendolo a $P_1$ ($P_2 \le P_1$).\\[3pt]
                \textsc{RisolviP$_2$}($x$)\\
                \hspace*{1.5em}$y \leftarrow \textsc{Riduci}(x)$\\
                \hspace*{1.5em}$sol \leftarrow \textsc{RisolviP$_1$}(y)$\\
                \hspace*{1.5em}\textbf{return} $sol$\\[3pt]
                \textbf{Caso indecidibile} Sia $P_1$ un noto problema indecidibile e sia $P_2$ un problema che si sospetta essere indecidibile. Si può dimostrare che $P_2$ è indecidibile riducendo $P_1$ a $P_2$ ($P_1 \le P_2$).\\[3pt]
                \textsc{RisolviP$_1$}($x$)\\
                \hspace*{1.5em}$y \leftarrow \textsc{Riduci}(x)$\\
        ...
        $O(g(n)) = \{f(n)\ |\ \exists c, n_0>0 \text{ tali che } \forall n \geq n_0, f(n) \leq c\cdot g(n)$\}
        ...
        $\Omega(g(n)) = \{f(n)\ |\ \exists c, n_0>0 \text{ tali che } \forall n \geq n_0, f(n) \geq c\cdot g(n)$\}
        ...
        $\Theta(g(n)) = \{ f(n)\ |\ \exists c_1, c_2, n_0 > 0 \text{ tali che } \forall n \geq n_0, \ c_1 \cdot g(n) \leq f(n) \leq c_2 \cdot g(n) \}$
                \\[5pt]
        ...
        $(a \times b)\mod n = [(a\mod n) \times (b\mod n)]\mod n$ \\
                ...
                \begin{multicols}{2}
                        ...
                        \verb|1|\hspace*{0.5em} \textbf{if} $lo < hi$\\
